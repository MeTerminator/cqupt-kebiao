...
async def update_cache(student_id: str):
    ...
    try:
        ...
        schedule_data = parse_jwzx_kebiao(html)
...
async def get_curriculum_data(student_id: str, background_tasks: BackgroundTasks) -> Optional[ScheduleSchema]:
    ...
    last_update = await redis_client.get(f"kebiao_html_ts:{student_id}")

    now = datetime.now().timestamp()

    # 2. 存在缓存的情况 (Stale-While-Revalidate)
    if cached_html and last_update:
        if now - float(last_update) > 5:
            # 超过 5s，触发后台异步刷新
            background_tasks.add_task(update_cache, student_id)

        # 无论是否过期，都先返回旧数据
        return parse_jwzx_kebiao(cached_html)
